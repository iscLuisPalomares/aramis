...
namespace ComprasProject {
    public partial class FilesCot : Form {
        public FilesCot() {
            InitializeComponent();
        }
        ...
        public void getdata_for_fileslist() {
            try {
                string query = "SELECT distinct(fsfilename) FROM tbcotfiles WHERE fsidcotizacion = '" + reqid + "'";
            ...
            if (listBox1.Items.Count > 0) {
                try {
                    ...
                    if (saveFileDialog1.ShowDialog() == DialogResult.OK) {
                        System.IO.File.Copy(@"\\mexfs01\TJTemp\Opardo\FOLIOS\cotizaciones\" + reqid + @"\" + listBox1.SelectedItem.ToString(),
