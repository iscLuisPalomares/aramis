...
namespace ComprasProject {
    public partial class FilesRequi : Form {
        public FilesRequi() {
            InitializeComponent();
        }
        ...
        public void getdata_for_fileslist() {
            try {
                string query = "SELECT _name FROM req_files WHERE _reqid = '" + reqid + "'";
            ...
            if (listBox1.Items.Count > 0) {
                try {
                    ...
                    if (saveFileDialog1.ShowDialog() == DialogResult.OK) {
                        System.IO.File.Copy(@"\\mexfs01\TJTemp\Opardo\FOLIOS\requisiciones\" + reqid + @"\" + listBox1.SelectedItem.ToString(),
