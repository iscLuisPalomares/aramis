...
namespace ComprasProject {
    public partial class ApproveAjuste : Form {
        public ApproveAjuste() {
            InitializeComponent();
        }
        ...
        private void disapproveajuste() {
            try {
                ...
                string disapproverequiquery = "UPDATE tbajustes SET " +
                    ...
                    + "update "
                    + "buckets set buckets.gasto = gastos.[Total cotizado dlls], buckets.asignado = 0 "
                    + "FROM buckets bucks "
                ...
                ejecucion.Parameters.AddWithValue("@user", usuario);
        ...
        private void aprobarajuste() {
            try {
                ...
                string sqlquery = "set xact_abort on \n BEGIN TRANSACTION;\n"
                    ...
                    + ", fsstatus = 'Ajuste Aprobado' WHERE fsid= @ajusteid;\n"
                    + "UPDATE tbajusteslineas SET fsstatus = 'Ajuste Aprobado' "
                    + "WHERE fsajusteid = @ajusteid;\n"
                    + "COMMIT TRANSACTION;";
                SqlCommand ejecucion    = new SqlCommand();
                ejecucion.Connection    = conn;
        ...
        private void sendmail() {
            ...
            mail.Subject = "Ajuste aprobado";
            mail.Body = "Se ha aprobado un nuevo ajuste, ingrese al Sistema ARAMIS por favor para continuar.";
            try {
                client.Send(mail);
            } catch (Exception) {
        ...
        private void sendmaildisapprove() {
            ...
            if (creadormail.Length == 0) {
                MessageBox.Show("User email not available.");
            } else {
                MailMessage mail = new MailMessage("aramis@posey.com", creadormail);
                SmtpClient client = new SmtpClient();
                client.Port = 25;
                ...
                client.UseDefaultCredentials = false;
                client.Host = "mail.posey.com";
                MotivoCotRechazada razon = new MotivoCotRechazada();
                razon.ShowDialog();
                mail.Subject = "Ajuste " + ajusteid + " desaprobado.";
                mail.Body = "Se ha denegado el ajuste " + ajusteid + ", ingrese al Sistema ARAMIS por favor para continuar.\n"
                    + "Razon:\n"
                    + razon.motivo;
                try {
                    client.Send(mail);
